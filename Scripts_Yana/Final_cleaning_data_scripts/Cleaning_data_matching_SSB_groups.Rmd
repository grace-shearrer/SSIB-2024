...
intDF[,c(1:8, 428)]<-sapply(intDF[,c(1:8, 428)], as.factor)
intDF[,c(9:427, 429:432)]<-sapply(intDF[,c(9:427, 429:432)], as.numeric)
highLow_DF[,c(9:427, 429:432)]<-scale(highLow_DF[,c(9:427, 429:432)], center = T, scale = F)
## Checking the center
round(colMeans(highLow_DF[,c(9:427, 429:432)]),2)
sapply(highLow_DF[,c(9:427, 429:432)], sd)
```

```{r}
write.table(highLow_DF, "~/OneDrive - University of Wyoming/0. Lab/M2AENAD Lab - Documents/RESEARCH/ABCD/Yana_SSIB_2024/data/matched111724_centered.csv",  row.names = FALSE, sep = ",")
```
