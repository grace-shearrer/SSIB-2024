...
```{r}
names(DF)
intDF <- DF[, c(1:5, 168:597)]
# Drop multiple columns by name
intDF <- intDF[, !names(intDF) %in% c("anthroweight1lb", "anthroweight2lb", "anthroweight3lb")]
```

#Drop missing values
```{r}
intDF<-intDF[complete.cases(intDF), ]
```
...
```{r}
DFhigh_low<-subset(intDF, intDF$ssb_group != "medium")
...
```{r}
#write.table(highLow_DF, "~/OneDrive - University of Wyoming/0. Lab/M2AENAD Lab - Documents/RESEARCH/ABCD/Yana_SSIB_2024/data/matched111724.csv",  row.names = FALSE, sep = ",")
```
