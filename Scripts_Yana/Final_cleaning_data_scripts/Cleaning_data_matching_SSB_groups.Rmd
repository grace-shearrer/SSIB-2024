...
library(ggplot2)
# Basic violin plot
ggplot(highlow_DFbak, aes(x = ssb_group, y = demo_comb_income_v2)) +
  geom_violin() +
  theme_minimal()
```
...
## 11/20/24
Household income is the most important even after matching. Now going to look at the dataset without matching but with increased household income metrics
```{r}
load(file = "~/OneDrive - University of Wyoming/0. Lab/M2AENAD Lab - Documents/RESEARCH/ABCD/Yana_SSIB_2024/data/finalDF.RData")
```

Keep only DFhigh_low
```{r}
rm(list = setdiff(ls(), "DFhigh_low"))
```

# Read in additional data from the 
https://doi.org/10.1016/j.dcn.2022.101164

```{r}
df1<-vroom("/Users/gracer/Library/CloudStorage/OneDrive-UniversityofWyoming/0. Lab/M2AENAD Lab - Documents/RESEARCH/ABCD/Yana_SSIB_2024/data/clean/ABCD_r.csv")
df2<-vroom("/Users/gracer/Library/CloudStorage/OneDrive-UniversityofWyoming/0. Lab/M2AENAD Lab - Documents/RESEARCH/ABCD/Yana_SSIB_2024/data/clean/ABCD_ses.csv")
```

```{r}
dfSES <- df1 %>%
  inner_join(df2, by = "subjectkey")
```

ses_fac is a computed factor score based on "Income2Needs","IncCombinedMidpoint","EdYearsHighest","EdYearsAverage"


# Randomly select one sibling from family 
```{r}
# Randomly select one row from each level of dfSES$rel_family_id
random_rows <- dfSES %>%
...
```{r}
dfTOTAL <- dfSES %>%
  inner_join(DFhigh_low, by = "subjectkey")
```
...
#Centering the data (again for b)
```{r}
df_cleaned[,c("crpbi_y_ss_parent", "srpf_y_ss_ses", "basic_needs_sum", "sub_crime_sum","Income2Needs","IncCombinedMidpoint","EdYearsHighest","EdYearsAverage", "reshist_addr1_adi_wsum", "ses_fac")]<-scale(df_cleaned[,c("crpbi_y_ss_parent", "srpf_y_ss_ses", "basic_needs_sum", "sub_crime_sum","Income2Needs","IncCombinedMidpoint","EdYearsHighest","EdYearsAverage", "reshist_addr1_adi_wsum", "ses_fac")], center = T, scale = F)
```

```{r}
df_cleaned <- dummy_cols(df_cleaned, select_columns = c("sex", "race","demo_prim","demo_prnt_prtnr_v2", "abcd_site_num"), remove_first_dummy = TRUE, remove_selected_columns = TRUE)
names(df_cleaned)
```


```{r}
write.table(df_cleaned, "~/OneDrive - University of Wyoming/0. Lab/M2AENAD Lab - Documents/RESEARCH/ABCD/Yana_SSIB_2024/data/DFSESnotmatched.csv", row.names = F, sep = ",")
```


```{r}
#save.image(file = "~/OneDrive - University of Wyoming/0. Lab/M2AENAD Lab - Documents/RESEARCH/ABCD/Yana_SSIB_2024/data/DFSESnotmatch.RData")
```